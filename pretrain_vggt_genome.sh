...
RESULTS="${PRETRAIN_OUT}/vsibench_results.jsonl"
BATCH_SIZE=128
NUM_WORKERS=12
...
python3 scripts/run_inference.py \
    --config vljepa/configs/inference_vggt.yaml \
    --checkpoint "$CKPT" \
    --manifest "$MANIFEST" \
...
results  = [json.loads(l) for l in open("outputs/pretrain_vggt_vg/vsibench_results.jsonl")]
