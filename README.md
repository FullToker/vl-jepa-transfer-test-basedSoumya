...
### 1.3 CPU-Only Setup (Recommended for local testing)

This path is stable for smoke tests and development.

```bash
pip install --index-url https://download.pytorch.org/whl/cpu torch torchvision
pip install Pillow PyYAML tqdm pytest
```

Optional (only needed for non-toy text/vision models):
...
Use this when training larger models or running paper-scale configs.
...
2. Install PyTorch CUDA wheels (example for CUDA 12.1):

```bash
pip install --index-url https://download.pytorch.org/whl/cu121 torch torchvision
pip install Pillow PyYAML tqdm pytest transformers
```

If your system uses a different CUDA build, choose the matching PyTorch index URL from official PyTorch install docs.
